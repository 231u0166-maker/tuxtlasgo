...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const srv = await pool.query(
      `SELECT id FROM servicios WHERE usuario_id=$1 ORDER BY creado_en DESC LIMIT 1`,
      [usuarioId]
    );
    if (srv.rows.length === 0) return res.status(404).json({ error: 'Sin servicio registrado' });
    const servicioId = srv.rows[0].id;

    const { nombre, categoria, municipio, descripcion, precio, contacto } = req.body;
    ...
    if (nombre?.trim()?.length >= 3)     { campos.push(`nombre=$${idx++}`);      valores.push(nombre.trim()); }
    if (categoria)                        { campos.push(`categoria=$${idx++}`);   valores.push(categoria); }
    if (municipio)                        { campos.push(`municipio=$${idx++}`);   valores.push(municipio); }
    if (descripcion?.trim()?.length >= 20){ campos.push(`descripcion=$${idx++}`); valores.push(descripcion.trim()); }
    if (precio?.trim())                   { campos.push(`precio=$${idx++}`);      valores.push(precio.trim()); }
    if (contacto?.trim())                 { campos.push(`contacto=$${idx++}`);    valores.push(contacto.trim()); }
    ...
    const r = await pool.query(
      `UPDATE servicios SET ${campos.join(', ')} WHERE id=$${idx} RETURNING *`,
      valores
    );
