...
import { getSQL, cors, getToken, verificarSesion, generarCodigo } from '../_db';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === 'POST') {
    ...
    if (!descripcion?.trim() || descripcion.trim().length < 20) return res.status(400).json({ error: 'Descripción muy corta' });
